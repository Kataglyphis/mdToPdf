...
RUN if [ "$TARGETARCH" = "amd64" ]; then \
    curl -L https://github.com/jgm/pandoc/releases/download/3.6/pandoc-3.6-1-amd64.deb \
    -o /tmp/pandoc.deb && \
    dpkg -i /tmp/pandoc.deb && \
    rm /tmp/pandoc.deb; \
    elif [ "$TARGETARCH" = "arm64" ]; then \
    curl -L https://github.com/jgm/pandoc/releases/download/3.6/pandoc-3.6-1-arm64.deb \
    -o /tmp/pandoc.deb && \
    dpkg -i /tmp/pandoc.deb && \
    rm /tmp/pandoc.deb; \
    fi
